...
export default function PoliciesPage() {
  ...
  const fetchDropdownData = useCallback(async () => {
    try {
      ...
      const [subjectsResponse, actionsResponse, resourcesResponse] = await Promise.all([
  ...
  const handleViewOpen = useCallback((policy: Policy) => {
      ...
      >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.8)', mt: 0.5 }}>
                {selectedPolicy ? 'Modify access control rules' : 'Define who can access what resources'}
              </Typography>
            </Box>
          </Box>
          <IconButton
                  ...
                  {selectedSubject ? (
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary" sx={{ mb: 2, display: 'block' }}>
                        Add specific conditions for when this policy applies to {subjects.find(s => s.id === selectedSubject)?.displayName}
                      </Typography>
                      
                      <Accordion sx={{ mb: 1, bgcolor: 'white', border: '1px solid', borderColor: 'grey.200' }}>
                        <AccordionSummary 
                        ...
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            <Typography variant="subtitle2" fontWeight="500">
                              {subjects.find(s => s.id === selectedSubject)?.displayName} - Conditions
                            </Typography>
                          </Box>
                        </AccordionSummary>
                          ...
                          <Grid container spacing={2}>
                            <Grid size={{ xs: 6 }}>
                              <FormControl fullWidth>
                                ...
                                <Select
                                  value={subjectAttributes?.type || ''}
                                  onChange={(e) => handleSubjectAttributeChange('type', e.target.value)}
                                  label="Type"
                                  size="small"
                                >
                            ...
                            <Grid size={{ xs: 6 }}>
                              <FormControl fullWidth>
                                ...
                                <Select
                                  value={subjectAttributes?.status || ''}
                                  onChange={(e) => handleSubjectAttributeChange('status', e.target.value)}
                                  label="Status"
                                  size="small"
                                >
                            ...
                            <Grid size={{ xs: 12 }}>
                              <TextField
                                fullWidth
                                label="Department"
                                value={subjectAttributes?.department || ''}
                                onChange={(e) => handleSubjectAttributeChange('department', e.target.value)}
                                placeholder="e.g., IT, HR, Finance (leave empty for any department)"
                                size="small"
                                helperText="Specify department requirement or leave empty"
                              />
                            </Grid>
                            <Grid size={{ xs: 12 }}>
                              <TextField
                                fullWidth
                                label="Role"
                                value={subjectAttributes?.role || ''}
                                onChange={(e) => handleSubjectAttributeChange('role', e.target.value)}
                                placeholder="e.g., admin, manager, employee (leave empty for any role)"
                                size="small"
                                helperText="Specify role requirement or leave empty"
                              />
                            </Grid>
                          </Grid>
                        </AccordionDetails>
        ...
        <DialogActions sx={{
          ...
          <Box sx={{ display: 'flex', gap: 1.5 }}>
            ...
            >
              {isSubmitting ? 'Creating...' : (selectedPolicy ? 'Update Policy' : 'Create Policy')}
            </Button>
          </Box>
        </DialogActions>
      </Dialog>
